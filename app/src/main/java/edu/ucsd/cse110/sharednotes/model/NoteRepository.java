...
public class NoteRepository {
    ...
    public NoteRepository(NoteDao dao, NoteAPI note_api) {
    ...
    public LiveData<Note> getSynced(String title) {
        ...
        Observer<Note> updateFromRemote = theirNote -> {
            ...
            if (theirNote.title == null) return; // do nothing
            if (ourNote == null || ourNote.version < theirNote.version) {
                Log.i("NOTE STRING GETSYNCED:", theirNote.toJSON());
    ...
    public void upsertLocal(Note note, boolean incrementVersion) {
        ...
        Log.i("NOTE STRING UPSERTLOCAL2:", note.toJSON());
        dao.upsert(note);
    }

    public void upsertLocal(Note note) {
        Log.i("NOTE STRING UPSERTLOCAL1:", note.toJSON());
    ...
    public LiveData<Note> getRemote(String title) {
        ...
        liveNote = new MutableLiveData<Note>();
        liveNote.postValue(note_api.getNoteAsync(title));

        var executor = Executors.newSingleThreadScheduledExecutor();
        poller = executor.scheduleAtFixedRate(()->{
            liveNote.postValue(note_api.getNote(title));
